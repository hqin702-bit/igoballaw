...
def today_meta():
    ...
    label_en = "%s, %s %d, %d" % (EN_DAYS[now.weekday()], EN_MONTHS[now.month-1], now.day, now.year)
    issue = now.timetuple().tm_yday  # 当年第几天，作为期号
    return label, label_en, issue, now.strftime("%Y.%m.%d")

PROMPT = """你是「律连全球 · iGlobal Law 国际商贸法律日报」的资深编辑（主笔为涉外律师秦林蒿）。
请基于你掌握的国际商贸、涉外法律、跨境合规领域的常识与典型议题，撰写今天（{date}）这一期日报的 4 条内容，**每条都要中英双语**。
要求：
...
2. 四个栏目各 1 条：
   ...
   - analysis（律师研判）：中文以「秦林蒿律师研判：」开头、英文以「Attorney Qin's view: 」开头，对一项规则或趋势给出实务建议。
3. 每条包含中文 title（25-45字）/summary（120-180字）/source，以及对应的英文 title_en / summary_en / source_en（地道专业英文，非直译腔）。
4. 内容须自洽、可信、不得编造具体的红头文件编号或不存在的判决书号；来源可写机构名+大致时间。
5. 仅输出 JSON，结构严格如下，不要任何额外文字：
{{
  "focus":    {{"title":"...","summary":"...","source":"来源：... · {dotdate}","title_en":"...","summary_en":"...","source_en":"Source: ... · {dotdate}"}},
...
def main():
    if not API_KEY:
        ...
        sys.exit(1)
    label, label_en, issue, dotdate = today_meta()
